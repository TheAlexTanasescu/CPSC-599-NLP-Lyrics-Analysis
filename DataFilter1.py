...
tag_stop_list0 = {'France', 'French Rap', 'Non-Music', 'Deutschland', 
                 'Deutscher Rap', 'Polska', 'Россия (Russia)', 'Polski Rap', 
                 'Русский рэп (Russian Rap)', 'En Español', 'Brasil',
                 ...
                 'België/Belgique', 'Italian Rap', 'Danmark', 'Turkey', 'Scandinavia', 'Jamaica'
                 }
...
# tag_list =  [tag_pair for tag_pair in tag_list if tag_pair[1] >= 100 and tag_pair[0] not in tag_stop_list0]

#Stop tag list1 
tag_stop_list1 = set([tag_pair[0] for tag_pair in tag_list if tag_pair[1] < 100])
# print(tag_stop_list1)
tag_stop_list = tag_stop_list0.union(tag_stop_list1)

df_merged.to_csv('ModData\\filter0.csv') 
df_merged_no_tags = df_merged[~df_merged['tags'].apply(lambda x: any(tag in tag_stop_list for tag in x))]
df_merged_no_tags.to_csv('ModData\\filter1.csv') #Removes tags we dont want 


unique_tags = dict()
for tags in df_merged_no_tags['tags']:
    for tag in tags:
        if tag in unique_tags:
            unique_tags[tag] = unique_tags[tag] + 1
        else:
